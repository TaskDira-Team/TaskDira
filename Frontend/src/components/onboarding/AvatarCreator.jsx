...
const A_THEME = {
  light: {
    ...
    tileLabel:
      "text-[10px] font-medium text-slate-600 text-center leading-tight",
    ...
    uploadRing:
      "w-20 h-20 rounded-2xl overflow-hidden ring-4 ring-indigo-300 shadow-lg",
    uploadIconBox:
      "w-14 h-14 rounded-full bg-indigo-100 flex items-center justify-center",
    uploadIcon: "h-7 w-7 text-indigo-500",
    uploadCta:
      "text-sm font-semibold text-indigo-700 flex items-center justify-center gap-1.5",
  ...
  dark: {
    ...
    uploadRing:
      "w-20 h-20 rounded-2xl overflow-hidden ring-4 ring-lime/50 shadow-lg",
    uploadIconBox:
      "w-14 h-14 rounded-full bg-lime/15 flex items-center justify-center",
    uploadIcon: "h-7 w-7 text-lime",
    uploadCta:
      "text-sm font-extrabold text-lime flex items-center justify-center gap-1.5",
...
export default function AvatarCreator({
  ...
  return (
    <div
      className={`avatar-creator space-y-5 ${compact ? "avatar-creator-compact" : ""}`}
    >
      <div className="avatar-creator-preview flex flex-col items-center gap-2 py-4">
        <AvatarFrame avatar={preview} size="xl" showGlow />
        <p className={at.badgeLabel}>{t(preview.profileBadgeKey)}</p>
        <p className={at.hint}>
          {t(preview.iconLabelKey)} · {t("avatar.ring")}:{" "}
          {t(preview.ringLabelKey)}
      ...
      {compact && (
        <div
          className="avatar-part-tabs"
          aria-label={
            lang === "he" ? "עיצוב הדמות" : "Customize your character"
          }
        >
          {[
            ["look", "Look", "דמות"],
      ...
      {(!compact || section === "look") && (
        <div className={`avatar-type-tabs ${at.tabBar}`}>
          {TAB_IDS.map((tab) => {
            const Icon = tab.icon;
            return (
              <button
                ...
                className={`flex-1 flex items-center justify-center gap-1.5 py-2 rounded-lg text-xs font-medium transition-all ${
                  activeTab === tab.id ? at.tabOn : at.tabOff
                }`}
              >
      ...
      {(!compact || section === "look") && activeTab === "emoji" && (
        <div>
          <label className={at.label}>{t("avatar.baseIcon")}</label>
          <div className="avatar-icon-grid grid grid-cols-4 sm:grid-cols-8 gap-2">
            {AVATAR_ICONS.map((icon) => (
              <button
                ...
                className={`text-2xl p-2 rounded-xl transition-all ${
                  local.avatarType === AVATAR_TYPES.EMOJI &&
                  local.baseIconId === icon.id
                    ? at.iconOn
                    : at.iconOff
                }`}
              >
      ...
      {(!compact || section === "look") && activeTab === "sticker" && (
        <div>
          <label className={at.label}>{t("avatar.gallery")}</label>
          <div className="avatar-sticker-grid grid grid-cols-3 sm:grid-cols-5 gap-3">
            {STICKER_PRESETS.map((sticker) => (
              <button
                ...
                className={`flex flex-col items-center gap-1.5 p-2 rounded-2xl border-2 transition-all ${
                  local.avatarType === AVATAR_TYPES.STICKER &&
                  local.stickerId === sticker.id
                    ? at.stickerOn
                    : at.stickerOff
                }`}
              >
                <div className={at.tile}>
                  {sticker.type === "gif" ? (
                    ...
                    <span className={`text-2xl ${sticker.animationClass}`}>
                      {sticker.emoji}
                    </span>
                  )}
                </div>
                <span className={at.tileLabel}>{t(sticker.labelKey)}</span>
              </button>
            ))}
          </div>
        </div>
      ...
      {(!compact || section === "look") && activeTab === "custom" && (
        <div className="space-y-3">
          <label className={at.labelPlain}>{t("avatar.uploadTitle")}</label>
          <input
            ref={fileRef}
            type="file"
          ...
          >
            {local.avatarType === AVATAR_TYPES.CUSTOM &&
            local.customImageData ? (
          ...
          {uploadError && <p className={at.uploadError}>{uploadError}</p>}
        </div>
      )}

      {(!compact || section === "color") && (
            ...
            {RING_COLORS.map((ring) => (
              <button
                ...
                className={`flex items-center gap-2 p-2.5 rounded-xl border-2 transition-all text-start ${
                  local.ringColorId === ring.id ? at.ringOn : at.ringOff
                }`}
              >
                <div
                  className={`w-8 h-8 rounded-full ${ring.bg} ${ring.ring.split(" ").slice(0, 2).join(" ")} shrink-0`}
                />
      ...
      {(!compact || section === "badge") && (
        <div className="avatar-badge-panel">
            ...
            {PROFILE_BADGES.map((badge) => (
              <button
                ...
                className={`py-2.5 px-3 rounded-xl text-sm font-medium border-2 transition-all text-start ${
                  local.profileBadgeId === badge.id ? at.badgeOn : at.badgeOff
                }`}
              >
                {t(badge.labelKey)}
              </button>
