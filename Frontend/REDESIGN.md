...
npm run dev
npm run build
node --test src/components/landing/demoState.test.js src/components/landing/tourContent.test.js src/components/landing/playgroundStore.test.js src/hooks/savedChores.test.js
```

GSAP 3.15.0 and Three.js 0.185.1 were added through npm, with package-lock.json updated. Verification uses the npm-locked dependencies (Vite 8.1.5). The 3D scene is dynamically imported only after the visitor chooses to explore. Its approximately 627KB minified chunk triggers Vite’s advisory size warning; it is not part of the initial landing download.

## What changed

- A full-screen miniature-home experience built with Three.js and GSAP: an opening roof and hinged door, animated camera travel, interactive room markers, a guided tour, floating floors, and a daylight/evening switch with warm interior lighting. Each room has a story and an animated chore. Completing dishes, plants, or laundry shares progress and points immediately with the playground; replaying an animation adds no points. The modal supports Escape, focus restoration, phone layouts, background scroll locking, and reduced motion. Rendering is on demand, and scenes dispose animations, geometry, materials, controls, and their WebGL context on close. Illustrated fallback and room controls remain available if WebGL fails.
